...
class SchedulingTest < ActiveSupport::TestCase
  def setup
    ...
    @scheduling1 = @user.schedulings.build(name: @workout.name, start_time: Time.zone.now.advance(hours: 1),
                                           workout_id: @workout.id)
    # mimics creating the scheduling from the Schedulings Index
    @scheduling2 = @user.schedulings.build(name: 'run', start_time: Time.zone.now.advance(days: 1))
