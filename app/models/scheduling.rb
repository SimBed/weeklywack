class Scheduling < ApplicationRecord
  ...
  scope :has_workout, -> { where.not(workout_id: nil) }
  scope :in_future, -> { where('start_time > ? AND workout_id IS NOT NULL', Time.zone.now.beginning_of_day) }
